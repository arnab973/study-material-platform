...
const ComputerScienceDatabasesNotes = () => {
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      try {
        const response = await fetch("/api/list-files?grade=ComputerScience&subject=databases&type=notes");
  ...
  return (
    <main className="p-6 min-h-screen">
      ...
      <h1 className="text-2xl sm:text-4xl font-bold mb-6 mt-6 text-white text-center">
        Computer Science <span className="text-primary">Databases</span> Notes
      </h1>
...
export default ComputerScienceDatabasesNotes;

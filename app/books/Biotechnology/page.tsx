    ...
    const fetchFiles = async () => {
      try {
        const response = await fetch(
          "/api/list-files?grade=Biotechnology&subject=books&type=books"
        );
  ...
  return (
    <main className="p-6 min-h-screen text-white">
      ...
      <h1 className="text-2xl sm:text-4xl font-bold mb-6 mt-6 text-white text-center">
        Biotechnology <span className="text-primary">Books</span>
      </h1>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-14">
          {files.map((file) => (
            <div
              key={file.filename}
              className="p-4 bg-white/10 rounded-lg border border-primary/50 flex flex-col"
            >
...
export default BiotechnologyBooksList;
